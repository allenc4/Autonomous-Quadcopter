...
#else
	while (*_echoInput & _echoBit)                // Wait for the ping echo.
		if (hal.scheduler->micros() > _max_time) return NO_ECHO;   // Stop the loop and return NO_ECHO (false) if we're beyond the set maximum distance.
#endif

	return (hal.scheduler->micros() - (_max_time - _maxEchoTime) - PING_OVERHEAD); // Calculate ping time, include overhead.
}
...
unsigned long NewPing::ping_median(uint8_t it) {
	...
	while (i < it) {
		t = hal.scheduler->micros();  // Start ping timestamp.
		last = ping(); // Send ping.
		...
		if (i < it && hal.scheduler->micros() - t < PING_MEDIAN_DELAY)
			hal.scheduler->delay((PING_MEDIAN_DELAY + t - hal.scheduler->micros()) / 1000); // Millisecond delay between pings.
...
bool NewPing::ping_trigger() {
	...
	*_triggerOutput &= ~_triggerBit; // Set the trigger pin low, should already be low, but this will make sure it is.
	hal.scheduler->delay_microseconds(4);            // Wait for pin to go low.
	*_triggerOutput |= _triggerBit;  // Set trigger pin high, this tells the sensor to send out a ping.
	hal.scheduler->delay_microseconds(10);           // Wait long enough for the sensor to realize the trigger pin is high. Sensor specs say to wait 10uS.
...
#else
	if (*_echoInput & _echoBit) return false;               // Previous ping hasn't finished, abort.
	_max_time = hal.scheduler->micros() + _maxEchoTime + MAX_SENSOR_DELAY; // Maximum time we'll wait for ping to start (most sensors are <450uS, the SRF06 can take up to 34,300uS!)
	while (!(*_echoInput & _echoBit))                       // Wait for ping to start.
		if (hal.scheduler->micros() > _max_time) return false;               // Took too long to start, abort.
#endif

	_max_time = hal.scheduler->micros() + _maxEchoTime; // Ping started, set the time-out.
...
bool NewPing::check_timer() {
	if (hal.scheduler->micros() > _max_time) { // Outside the time-out limit.
...
#endif
		timer_stop();                  // Disable timer interrupt
		ping_result = (hal.scheduler->micros() - (_max_time - _maxEchoTime) - PING_TIMER_OVERHEAD); // Calculate ping time including overhead.
